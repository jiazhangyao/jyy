...
import { CHATROOM_SDK } from '@/config/polyv';
import { getPolyvChatToken, getPolyvChatHistory, getPolyvChatStudentQuestionHistory, getVideoSocket, getGiftList, getPlanDetailInfo } from '@/server/Video';
...
@Component
export default class TeacherLiveMixins extends Vue {
    // 聊天室对象
    ...
    // 在线学员单页数量
    protected onlineUserListPageCount: number = 40;
    // 在线学员当前页码
    ...
    // 滚动加载在线学员标记
    protected scrollLoadingFLag: boolean = false;
    // 提问记录
    ...
    // 回复框显示状态
    protected teacherAnswerShowStatus: boolean = false;
    // 回复框高度
    protected teacherAnswerHeight: number = 162;
    // 课次信息
    ...
    protected initChatRoom(chatRoomToken: { mediaChannelKey: string, token: string }): void {
        ...
        Promise.all([
            ...
            return Promise.all([
                ...
                this.loading = false;
                this.teacherAnswerHeight = (document.getElementById('teacherAnswer') as Element).clientHeight;
            });
        }).catch(err => {
            console.log(err);
            this.loading = false;
    ...
    protected getHistoryMessage(): Promise<any> {
        ...
        return getPolyvChatHistory(this.roomId.toString(), Dayjs(this.planInfo.startTime * 1000).format('YYYY-MM-DD HH:mm:ss'), Dayjs(this.startTimeStamp).format('YYYY-MM-DD HH:mm:ss')).then(res => {
     ...
     * @memberof PolyvChatRoomMixins
     */
    protected reloadOnlineUserList(): void {
        this.scrollLoadingFLag = true;
        // 防抖处理
        clearTimeout(this.onlineUserListReloadTimer);
        this.onlineUserListReloadTimer = setTimeout(() => {
            // 重置在线学员列表
            this.onlineUserListPageNumber = 1;
            this.onlineUserListPageCount = 40;
            this.onlineUserList = [];
            this.handleGetOnlineUserList({ onlineUserListPageNumber: this.onlineUserListPageNumber, onlineUserListPageCount: this.onlineUserListPageCount });
            (this.onlineUser as any).scrollToTop();
    ...
    protected handleShieldStatusChange(data: any): void {
        console.log(data, this.onlineUserList);
        if (data.data.banType === 'userId') {
            ...
            console.log(index);
            if (index !== -1) {
                this.onlineUserList[index].banned = data.EVENT === "ADD_SHIELD" ? true : false;
            }
        }
    ...
    protected handleSelectQuestion(e: any): void {
        ...
        if (e.event === 'S_QUESTION' || e.EVENT === 'S_QUESTION') {
            this.teacherAnswerShowStatus = true;
     ...
     * @memberof ChatRoomMixins
     */
    protected handlePageClick(e: MouseEvent): void {
        this.teacherAnswerHeight = (document.getElementById('teacherAnswer') as Element).clientHeight;
        this.teacherAnswerShowStatus = false;
    }
