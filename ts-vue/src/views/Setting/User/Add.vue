    ...
    <a-form :form="form" @submit="handleFormSubmit" v-if="!loading">
      ...
      >
        <a-input
          ...
          class="int"
          v-decorator="['password', { initialValue: passworld,rules: [{ required: true, message: '请输入密码' },{ pattern: $data.regPassworld, message: '密码至少6位，不支持中文符号' }]}]"
        />
      </a-form-item>
      <a-form-item label="角色" :label-col="{ span: 6 }" :wrapper-col="{ span: 12 }" required>
        <a-select class="int"
                  v-decorator="['role',{ initialValue: role, rules: [{ required: true, message: '请选择角色' }]}]">
...
import { getGroups, getRoles, addUser } from "@/server/setting/user";
import { Form } from "ant-design-vue";
import { userName, name, password } from "@/common/js/regExp";
import { beforeRouteLeaveModal } from '@/common/js/modal';
@Component({
  beforeRouteLeave(to: any, from: any, next: any): void {

    if ((this as UserAdd).beforeRouteLeaveConfig.submit) {
      next();
      return;
    }
    // @ts-ignore
    const formValues: any = this.form.getFieldsValue();
    if (formValues.userName || formValues.name || formValues.password || formValues.group) {
...
export default class UserAdd extends Vue {
  ...
  private regPassworld: RegExp = password;
  ...
  private handleFormSubmit(e: any): void {
    ...
    this.form.validateFields((err, values) => {
      if (!err) {
        ...
        addUser(values.userName , values.name, values.password, values.role, values.group, this.status).then(res => {
  ...
  private getRolesList(): void {
    getRoles().then(res => {
      ...
      this.role = this.rolesList.length ? this.rolesList[0].id : '';
    }).catch(err => {
      this.$message.error(err.data.errmsg);
    });
  }
