...
import {ratePayTit, ratePayInfo, obligorSellerTit, obligeeBuyerTit, fsTit} from "./const"
...
class TaxDetail extends DetailPage {
  ...
  getTaxInfo = (entity, registerType) => {
    const {info = [], obligorDetail = [], obligeeDetail = [], nonTaxDTO = {}} = entity;
    ...
    return (
      <div>
        {/* mock数据，接口通后修改 */}
        <Card title='非税信息' className="forDetailTable" extra={<span {...styleProps}>单位（元）</span>}>
          {this.getTableInfo([nonTaxDTO], fsTit)}
        </Card>
        <Card title='税费信息' className="forDetailTable" extra={<span {...styleProps}>单位（元）</span>}>
          { this.getTableInfo(arr, ratePayInfo)}
        </Card>
  ...
  doRender() {
    const {store: {data, entity: {registerType}}} = this.props;
    return (
