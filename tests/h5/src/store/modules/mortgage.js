import { transferConfirmInfo, upload, mortgageApplicationSign, contractSign } from "../../service/mortgage";
	...
	actions: {
		...
		async uploads({rootGetters, rootState, state}, params) {
			...
			if (success) {
				...
				if (true||adjustStr.names == '1') {
					const { success, data } = await mortgageApplicationSign(contractInfo)
					if (success) {
						Dialog.alert({
							title: '办理成功',
							message: '您本次业务已成功办理'
