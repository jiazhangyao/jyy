import { transferPretaxInfo, transferPreTaxConfirm, transferPreTaxConfirms } from "../../service/transfer";
...
export default {
    ...
    actions: {
        async getPretaxInfo({ commit, rootState }, params) {
            const { buyerOrSeller, paytaxStatus, registerType } = rootState.transfer;
            const { success, data, msg, fieldErrors } = await transferPretaxInfo(params);
            if (success) {
