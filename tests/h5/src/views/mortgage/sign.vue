...
export default {
        ...
        signText = this.$store.state.applyinfo.adjustSigns
			...
			if (Object.keys(signText).length > 0) {
				for (let i in signText) {
					if (i == this.$store.state.mortgage.signType) {
						document.getElementById('pdfKey').value = signMap.get(i) + (signText[i] + 1)	
					}
				}
			} else {
        document.getElementById('pdfKey').value = signMap.get(String(this.$store.state.mortgage.signType)) + 1
			}
  ...
  methods: {
    signs() {
      ...
      this.$store.dispatch('mortgage/uploads', form,)
    },
    cancel() {
      cancelSign()
      if (this.$route.params.names === 1) {
        this.$router.push('/mortgage/apply')
      } else {
        this.$router.push('/mortgage/contract')
      }
    },
    clear() {
      clear_canvas()
