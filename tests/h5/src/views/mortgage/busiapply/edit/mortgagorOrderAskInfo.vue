<template>
  <div>
    ...
    <ul>
      <li>
        <div class="question">1.申请事项是否为申请人真实意思表示？</div>
        <div class="answer">
      ...
      <li>
        ...
        <div
        class="owner-box"
          v-if="haveJoinOwner==1 && (userRole == UserRoleEnum.getValueFromAlias('ROLE_ASSIGNEE')||userRole == UserRoleEnum.getValueFromAlias('ROLE_APPLICANT'))"
        >
          <div v-for="(item, index) in applicantList" :key="item.name" class="applicant">
            <div>
              ...
              <div class="del-btn" v-if="canEditApplicant && item.isApplicant==0" @click="delApplicant(index)">删除</div>
            </div>
          </div>
        </div>
      </li>
      <li>
        <div class="question">3.是否将不动产的权利状况和其他状况向抵押权人作了如实陈述，并保证无争议？？</div>
        <div class="answer">
          <template v-if="canEdit()">
      ...
      <li>
        <div class="question">4.是否保证所提交的申请登记材料、申请信息真实、合法、有效，如有不实，愿承担一切法律责任，与登记机构无关？</div>
        <div class="answer">
          <template v-if="canEdit()">
            <button :class="isLaw==1 ? '' : 'not'">是</button>
            <button @click="onItemClick('isLaw',0)" :class="isLaw==0 ? '' : 'not'">否</button>
...
export default {
  ...
  methods: {
    onItemClick(param, value) {
      console.log(" onItemClick(param, value) ",param, value)
      Dialog.confirm({
        title: "提示",
  ...
  mounted() {
    this.applicantList = this.transferbusiapplyForm.buyerInfo
      ? this.transferbusiapplyForm.buyerInfo.applicantList
      : [];
    this.haveJoinOwner = (() => {
