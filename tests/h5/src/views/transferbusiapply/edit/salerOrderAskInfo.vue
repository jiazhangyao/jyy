  ...
  <div>
    ...
    <ul>
      ...
      <li>
        ...
        <div class="answer">
          <template v-if="canEdit()">
            <button @click="haveJoinOwner=1" :class="haveJoinOwner==1 ? '' : 'not'">是</button>
            <button @click="haveNoJoinOwner" :class="haveJoinOwner==0 ? '' : 'not'">否</button>
          </template>
          <template v-else>{{haveJoinOwner==1?"是":"否"}}</template>
        </div>
        <div class="owner-box" v-if="haveJoinOwner==1 && userRole == UserRoleEnum.getValueFromAlias('ROLE_ASSIGNOR')">
          <div v-for="(item, index) in applicantList" :key="item.name" class="applicant">
            <div>
              ...
              <div class="del-btn" v-if="canEdit() && item.isStarter==0" @click="delApplicant(index)">删除</div>
            </div>
          </div>
          <Button class="add-btn" v-if="canEdit() && applicantList.length<5" @click="addapplicant">新增共有人</Button>
        </div>
      </li>
      <li>
...
export default {
  ...
  methods: {
    haveNoJoinOwner(){
      let abligors = this.applicantList.filter((item)=>item.isStarter == 1);
      this.haveJoinOwner=abligors.length>1 ? 1 : 0;
    },
    ...
    finish() {
      let postData = {
        salerJoinOwner: { applicantList: this.applicantList },
        salerOrderAskInfo: {
          isReal: this.isReal,
  ...
  mounted() {
    const {salerJoinOwner = {applicantList:[]}, salerJoinOwner:{applicantList}} = this.transferbusiapplyForm;
    this.applicantList = applicantList || [];

    this.haveJoinOwner = (() => {
      if (!this.applicantList.length) {
        return 0;
      } else if (this.applicantList[0].orgType == 2) {
        return 0;
      } else if (this.applicantList.length > 1) {
        return 1;
      } else {
        return 0;
      }
