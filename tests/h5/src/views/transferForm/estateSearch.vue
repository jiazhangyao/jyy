  ...
  <div class="estate-search">
    ...
    <div class="estate-search-wrap">
      ...
      <AutoForm v-model="form1Data" :data="firstData" @save="saveFirst" v-if="step === 1">
        <div slot="ancestralLandCode" class="hint" v-if="tag == 'land'">
          <p>1.平房或独幢楼房没有进行实地查看的，请前往不动产登记中心办理实地勘察。</p>
          <p>2.若没有进行权籍调查，请前往不动产登记中心申请权籍调查获取宗地代码。</p>
          <p>3.土地转移登记发起方必须是产权人之一。</p>
        </div>
        <div slot="warrantNumber" class="hint" v-if="tag === 'house'">
...
export default {
  ...
  created() {
    ...
    this.firstData = this.tag === "land" ? this.data3 : this.data1;
  },
  computed: {
    pageTitle() {
  ...
  methods: {
    ...
    saveFirst(data) {
      ...
      if (this.tag === "land") {
        ...
        Dialog.alert({
          title: "提示",
          message: "土地转移登记发起方必须是产权人之一,请知悉！",
          confirmButtonText: "继续办理"
        }).then(() => {
          this.step++;
          this.registerType = 5010;
        });
      } else {
        // 房屋
