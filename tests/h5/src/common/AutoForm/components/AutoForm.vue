    ...
    >
        ...
        <template v-else-if="item.type === 3">
            <van-cell
                ...
                "
                is-link
                :value="item.adjustSel === 0 ? '请选择' : item.data[item.value] ? item.data[item.value] : '请选择'"
                @click.native="() => {
                    item.disabled || (item.show = true); 
                    item.adjustSel = 1
                } "
            />
            <Actionsheet
...
export default {
    ...
    methods: {
        ...
        confirm_Actionsheet(value, index, item) {
            console.log('选中哪个', value, typeof value);
            item.callback && item.callback(value, index, item)
            this.form[item.key] = value
            
            // if (typeof value == object) {
            //   this.form[item.key] = value.text
            // }
            // else {
            //   this.form[item.key] = value
            // }
