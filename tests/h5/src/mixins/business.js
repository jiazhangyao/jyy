import {
  idCardValidate,
  submitTransOrderBySaler,//卖家提交
  submitTransOrderByBuyer,//买家
  addTransOrderBySaler, //卖家暂存
  addTransOrderByBuyer, //买家 
  submitTransOrderByNewHouse, //新房提交
  addTransOrderByNewHouse,  //新房暂存
  endTransOrderBySaler, // 终止
} from "@/service/business";

import { Toast } from "vant";
import { tabItems, tabsRelateToUserRole, UserRoleEnum } from "@/views/transferbusiapply/const.js";
...
const businessMix = {
  ...
  computed: {
    ...mapState("transferbusiapply", ["transferbusiapplyForm", "whichEdit", "materialTypeList", "pageEditable"]),
    hasownedWay() {
      let form = this.$store.state.transferbusiapply.transferbusiapplyForm;
      return (
        form.registerType == 1 ||
        form.registerType == 8 ||
        form.registerType == 9 ||
        form.registerType == 3 ||
        form.registerType == 15
      );
    },
    componentFinished() {
      let form = this.$store.state.transferbusiapply.transferbusiapplyForm;
      ...
      let res = {
        ...
        salerInfo: this.salerInfoValid(form.salerInfo).result,
        salerInfoForBuyer: this.salerInfoForBuyerValid(form.salerInfo).result,
    ...
    items() {
      let items = [];
      let { tabs = [] } = tabsRelateToUserRole[this.transferbusiapplyForm.userRole] || {};
  ...
  methods: {
    ...mapMutations("transferbusiapply", ["setWhichEdit", "setComponentTitle", "setPageEditable", "setMaterialTypeList"]),
    // 个人 权利人证件类型中文名
    cardTypeDesc(key) {
    ...
    updateFamilyMember(value) {
      ...
      this.familyList[value.familyIndex].familyMemberList = [
        ...this.applicantList,
        ...this.familyList[value.familyIndex].familyMemberList.filter(item=>item.isApplicant==0)
      ]
    ...
    async endTransOrder() {
      ...
      const res = await endTransOrderBySaler({ id: id });
    ...
    goPrev() {
      this.$store.dispatch('transferbusiapply/getUserRole').then((userRole) => {
        let { tabs } = tabsRelateToUserRole[userRole];
    ...
    // 暂存
    stage() {
      let transferbusiapplyForm = this.$store.state.transferbusiapply.transferbusiapplyForm;
      return new Promise((resolve, reject) => {
        let postData = this.formDataToBackend(transferbusiapplyForm);
        if (transferbusiapplyForm.registerType == 5020) {
          addTransOrderByNewHouse(postData).then(res => {
            if (res.success) {
        ...
        } else if (this.$route.query.id && transferbusiapplyForm.loginUserType == 2) { // 通过id判断是不是编辑，有id是编辑，loginUserType为2是买方

          addTransOrderByBuyer(postData).then(res => {
            if (res.success) {
              if (!transferbusiapplyForm.id) {
                transferbusiapplyForm.id = res.data;
                transferbusiapplyForm.orderTradeInfoDTO.orderId = res.data;
        ...
        } else {// 通过id判断是不是编辑，没有id是新增为卖方
          addTransOrderBySaler(postData).then(res => {
            if (res.success) {
              if (!transferbusiapplyForm.id) {
                transferbusiapplyForm.id = res.data;
                transferbusiapplyForm.orderTradeInfoDTO.orderId = res.data;
    ...
    submit() {
      ...
      return new Promise((resolve, reject) => {
        let postData = this.formDataToBackend(transferbusiapplyForm);
        if (transferbusiapplyForm.registerType == 5020) { // 一手房
          submitTransOrderByNewHouse(postData).then(res => {
            if (res.success) {
              resolve();
            } else {
              reject(res);
          ...
          submitTransOrderByBuyer(postData).then(res => {
            if (res.success) {
              resolve();
            } else {
              reject(res);
          ...
          submitTransOrderBySaler(postData).then(res => {
            if (res.success) {
              resolve();
            } else {
              reject(res);
    ...
    */
    ...
    salerInfoForBuyerValid() {
      return { result: true };
    },
    salerInfoValid() {
      let salerInfo = this.transferbusiapplyForm.salerInfo || {};
      let applicantList = salerInfo.applicantList || [];
